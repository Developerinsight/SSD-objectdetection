...
# "utils" 폴더의 data_augumentation.py를 import. 입력 이미지의 전처리를 수행하는 클래스
...
def make_datapath_list(rootpath):
    ...
    for line in open(train_id_names):
        ...
        img_path = (imgpath_template % file_id)  # 이미지의 경로
...
class Anno_xml2list(object):
    ...
    def __call__(self, xml_path, width, height):
        ...
        # 이미지 내 모든 물체의 어노테이션을 이 리스트에 저장합니다
        ret = []

        # xml 파일을 로드
        xml = ET.parse(xml_path).getroot()

        # 이미지 내에 있는 물체(object)의 수만큼 반복
...
Image Transformations: Resize, Rotate
    ...
    def pull_item(self, index):
        '''전처리한 이미지의 텐서 형식 데이터, 어노테이션, 이미지의 높이, 폭을 취득한다'''

        # 1. 이미지 읽기
        image_file_path = self.img_list[index]
        img = cv2.imread(image_file_path)  # [높이][폭][색BGR]
        height, width, channels = img.shape  # 이미지의 크기 취득
...
def od_collate_fn(batch):
    ...
    imgs = []
    for sample in batch:
        imgs.append(sample[0])  # sample[0] 은 이미지img입니다
...
def make_extras():
    ...
    cfg = [256, 512, 128, 256, 128, 256, 128, 256, 128, 256]
    ...
    layers += [nn.Conv2d(cfg[6], cfg[7], kernel_size=(1))]
    
    layers += [nn.Conv2d(cfg[7], cfg[8], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[8], cfg[9], kernel_size=(3))]
    return nn.ModuleList(layers)


# 디폴트 박스의 오프셋을 출력하는 loc_layers,
# 디폴트 박스에 대한 각 클래스의 신뢰도 confidence를 출력하는 conf_layers를 작성
def make_loc_conf(num_classes=21, bbox_aspect_num=[4, 6, 6, 6, 6, 4, 4, 4]):
    ...
    # extra(source2)에 대한 합성곱층
    loc_layers += [nn.Conv2d(512, bbox_aspect_num[1]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(512, bbox_aspect_num[1]
                              * num_classes, kernel_size=3, padding=1)]
    
    # VGG의 최종층(source3)에 대한 합성곱층
    loc_layers += [nn.Conv2d(1024, bbox_aspect_num[2]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(1024, bbox_aspect_num[2]
                              * num_classes, kernel_size=3, padding=1)]
    ...
    # extra(source7)에 대한 합성곱층
    loc_layers += [nn.Conv2d(256, bbox_aspect_num[7]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(256, bbox_aspect_num[7]
                              * num_classes, kernel_size=3, padding=1)]
...
class SSD(nn.Module):
    ...
    def forward(self, x):
        sources = list()  # loc와 conf로의 입력 source1-7을 저장
        ...
        # conv5_3의 출력을 L2Norm에 입력하고, source2을 작성하여, sources에 추가
        source2 = self.L2Norm(x)
